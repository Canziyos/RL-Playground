...
from src.discretization import discretize_disturbed_state
from src.envs.disturbed_cartpole import DisturbedCartPoleSim


CONFIG_PATH = "configs/disturbed.json"
Q_TABLE_PATH = "outputs/disturbed/q_table.pkl"
...
def main():
    ...
    state_cont = env.reset(drunk=bool(config["eval_drunk_initial_state"]))
    ...
    ax.set_aspect("equal")
    ax.set_title("Disturbed CartPole")
    ...
    def update(frame):
        ...
        env_action = map_agent_action(action)
    ...
    anim = animation.FuncAnimation(
        ...
        interval=int(env.dt * MILLISECONDS_PER_SECOND),
        blit=False,
        repeat=False,
    )
